...
get_alcohol_spatial <- function(data, input){
  reactive(
    ...
    mutate(increase_decrease = case_when( 
      increase_decrease == "TRUE" ~ "Increase From Previous Year",
      increase_decrease == "FALSE" ~ "Decrease From Previous Year")) %>% 
    mutate(increase_decrease = factor(increase_decrease, 
                                      levels = c("Increase From Previous Year", "Decrease From Previous Year"))) %>% 
