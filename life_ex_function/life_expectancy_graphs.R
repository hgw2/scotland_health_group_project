...
life_expectancy_comparison_plot <- life_expectancy_comparrison %>%
  arrange(council_name) %>%
  ggplot() +
...
## plot 3 - top 5 councils with highest life expectancy 2017
...
## plot 4 - bottom 5 councils with lowest life expectancy in 2017

bottom_five <- life_expectancy %>%
  select(council_name, sex, life_expectancy, year) %>%
  filter(year == 2017, council_name != "Scotland Wide") %>%
